...
int GenAlertKey(int argc, char *argv[])
{
    ...
    // Sign
    const char* pszPrivKey = "HEX PRIVATE KEY GOES HERE!";
    std::vector<unsigned char> vchTmp(ParseHex(pszPrivKey));

    CKey key;
    key.SetSecret(&vchTmp.front());
    ...
    WriteTests(testoutdir / "alertTests.raw", alerts);

    std::string outPriv;
    outPriv = "// Private alert key header - CONFIDENTIAL\n\n";
    outPriv += "const char* psz PrivKey = \"";
    outPriv += HexStr(privkey);
    outPriv += "\";\n";

    outPriv += "// Add the following to chainparams.cpp constructor\n";
