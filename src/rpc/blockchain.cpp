...
UniValue getstakemodifiercheckpoints(const JSONRPCRequest& request)
{
    ...
    while (nHeight < chainActive.Height()) {
        ...
        result.push_back(Pair(strprintf("%d", nHeight+i), strprintf("%08x hash: %s PoWHeight: %d", pblockindex->nStakeModifierChecksum, pblockindex->GetBlockHash().GetHex(), GetPowHeight(pblockindex)-1)));
        nHeight += 50000;
    }
