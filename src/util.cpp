...
const char * const BITCOIN_CONF_FILENAME = "DeepOnion.conf";
const char * const BITCOIN_PID_FILENAME = "DeepOniond.pid";
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/DeepOnion";
#else
    // Unix
...
/* Parse an octal number, ignoring leading and trailing nonsense. */
 int
archive::parseoct(const char *p, size_t n)
{
    int i = 0;
...
int
archive::is_end_of_archive(const char *p)
{
    int n;
    for (n = 511; n >= 0; --n)
        if (p[n] != '\0')
...
void
archive::create_dir(char *pathname, int mode)
{
    char *p;
    int r;
...
int
archive::verify_checksum(const char *p)
{
    int n, u = 0;
    for (n = 0; n < 512; ++n) {
        if (n < 148 || n > 155)
